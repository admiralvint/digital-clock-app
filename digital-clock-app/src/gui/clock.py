...
class Clock:
    ...
    def __init__(self, master):
        ...
        self.label_btc = tk.Label(master, font=self.font_crypto, bg="#0d0d0d", fg="#FFD700")  # Neon yellow
        self.label_btc.pack(expand=True, fill='both')
        
        self.label_eth = tk.Label(master, font=self.font_crypto, bg="#0d0d0d", fg="#FFD700")  # Neon yellow
    ...
    def update_crypto_prices(self):
        ...
        self.master.after(300000, self.update_crypto_prices)  # Update every 5 minutes (300,000 milliseconds)
    ...
    def resize_font(self, event):
        ...
        new_size_crypto = max(min(event.width // 40, event.height // 10), 15)  # Ensure a minimum font size of 10
